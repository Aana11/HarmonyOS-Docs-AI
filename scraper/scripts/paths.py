...
def url_to_reference_relative(url: str, from_md_path: Path, references_root: Path,
    ...
    # Markdown links always use forward slashes, including when the scraper
    # runs on Windows where os.path.relpath() returns backslashes.
    rel = Path(os.path.relpath(str(target), start=str(from_md_path.parent))).as_posix()
    if parsed.fragment:
